...
$centros = $dal->query($dal->c,$sql_cen);
  foreach ($centros as $cen){
	...
	$cabecera_alumnos="select 'nombre','primer_apellido','segundo apellido','telefono','email','codigo ciclo','denominacion ciclo','provincia','grado' union ";
	$sql_brutos_alumnos=$cabecera_alumnos."select nombre,primer_apellido,segundo_apellido,telefono,email,codciclo,denciclo,c.provincia as provincia,ci.grado as grado from alumnos a left join respuestas r on a.idalumnofct=r.idalumnofct, centros c,ciclos ci where ci.idciclofct=a.idciclofct and a.idcentrofct=c.idcentrofct and c.idcentrofct=".$param."  into outfile '/home/fpleaks/tmp/brutos.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n'";
	...
	$cabecera_respuestas="select 'nombre','primer_apellido','segundo apellido','telefono','email','codigo ciclo','denominacion ciclo','provincia','grado','fct','trabaja','relacionado con el estudio','tipo de contrato','misma empresa' union ";
	$sql_brutos_respuestas=$cabecera_respuestas."select nombre,primer_apellido,segundo_apellido,telefono,email,codciclo,denciclo,c.provincia as provincia,ci.grado as grado,fct,trabaja,relacionado,contrato,mismaempresa from alumnos a left join respuestas r on a.idalumnofct=r.idalumnofct, centros c,ciclos ci where ci.idciclofct=a.idciclofct and a.idcentrofct=c.idcentrofct and c.idcentrofct=".$param."  into outfile '/home/fpleaks/tmp/brutos.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n'";
...
//ESTADISTICA POR CENTROS
$sql_cen="SELECT distinct(idcentrofct) as idcentrofct, nombrecentro from alumnos a,centros c where a.idcentrofct=c.idcentrofct"; 
$cont=0; 
$centros = $dal->query($dal->c,$sql_cen);
