...
class ACCESO {
	...
	private $csv_dir='';
	private $periodo_dir='';
	public function __construct($dir,$dirp)
	{
		$this->csv_dir=$dir;
		$this->periodo_dir=$dirp;
	...
	$res = mysqli_query($this->c,$sql);
        ...
        copy('/home/fpleaks/tmp/brutos.csv',$fcentro);
        return 1;
        }

	public function gen_csv($sql,$param,$fichero,$ncentro='')
	{
 		$dir=$this->periodo_dir;
		$fp = fopen('tmp.csv', 'w');
		$csv='';
		$csvnames='';
		...
		$res = mysqli_query($this->c,$sql);
	        if(!$res)
			{
		      	print("NO hay resultado ".mysqli_error($this->c));
        		return null;
			}
		$results = array();
		...
		$str=str_replace('"', "",$str);
		$dircentro=$dir."/".$ncentro;
		if (!file_exists($dircentro)) 
		{
		    mkdir($dircentro, 0777, true);
		}
		$fcompleto=$dir."/".$ncentro.'/'.$fichero.".csv";
		echo $str;
		echo file_put_contents($fcompleto, $str);
    return;        
  } 
